...
- iterate throguh array `nums` and count number of operation `n`
- for each subarray `nums[i:i+3]`
	- if `nums[i]` == 1 : no operation
	- else : flip subarray (`[1-x for x in subarray]`), `n++`
- return `n` if `all(nums)` or last two bits are both 1 else `-1`
